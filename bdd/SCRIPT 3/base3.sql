...
Create table recette(
    ...
    iddevis int references devis(id),
    idlieu  int references lieu(id),
    idcategorie_place int  references categorie_place(id),
    nombrevendu int,
    daterecette timestamp default current_date,
    etat int default 0
);
...
select d.*,pl.idlieu,l.lieu,cp.id as idcategorie_place,cp.categorie_place,pl.nombreplace from devisplace d join place_lieu pl on d.idplace=pl.id join lieu l on pl.idlieu=l.id join categorie_place cp on pl.idcategorie_place=cp.id;



-- Recette reelle
drop view v_liste_recette_reelle;
Create or replace  view v_liste_recette_reelle as
select r.id,r.idcategorie_place,r.iddevis,r.idlieu,vlp.prix,r.nombrevendu,(r.nombrevendu*vlp.prix) as montant from recette  r join v_liste_devisplace vlp on r.idcategorie_place=vlp.idcategorie_place group by r.id,r.idcategorie_place,r.iddevis,r.idlieu,vlp.prix,r.nombrevendu;

-- Total depense
--+5
drop view total_devis_artiste;
Create or replace  view total_devis_artiste as
select iddevis,sum(heureartiste*tarif_heure) from  v_liste_devisartiste group by  iddevis;

drop view total_devis_sono;
Create or replace  view total_devis_sono as
select iddevis,sum(heuresonorisation*tarif_heure*quantite) from  v_liste_devissono group by  iddevis;

drop view total_devis_log;
Create or replace  view total_devis_log as
select iddevis,sum(jour*tarif_jour*quantite) from  v_liste_devislogistics group by  iddevis;

drop view total_devis_dep;
Create or replace  view total_devis_dep as
select iddevis,sum(prix) from  v_liste_devisdepense group by  iddevis;

drop view total_devis_lieu;
Create or replace  view total_devis_lieu as
select iddevis,sum(prix) from  v_liste_devislieu group by  iddevis;

drop view v_liste_total_depense;
Create or replace view v_liste_total_depense as
select  a.iddevis,a.sum+s.sum+l.sum+d.sum+li.sum as total_depense from total_devis_artiste a join total_devis_sono s  using(iddevis) join total_devis_log l using(iddevis) join total_devis_dep d using(iddevis) join total_devis_lieu li using(iddevis) group by a.iddevis,a.sum,s.sum,l.sum,d.sum,li.sum;

drop view stat_spectacle;
Create or replace view stat_spectacle as
select vr.iddevis,vde.nom,vde.date_debut,vde.date_fin,sum(vr.montant) as recette,vd.total_depense from v_liste_recette_reelle vr join v_liste_total_depense vd using(iddevis) join v_liste_devis vde on vr.iddevis=vde.id group by  vr.iddevis,vd.total_depense,vde.nom,vde.date_debut,vde.date_fin;
